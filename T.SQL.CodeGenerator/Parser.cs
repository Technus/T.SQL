...
internal class Parser
{
    ...
    {
        var (table, implementing) = GetMembers(classy);
        return new(
          ...
          tableMemberDeclarations: table,
          implementingTypes: implementing
        );
    }

    private (List<ISymbol> table, List<ITypeSymbol> implementing) GetMembers(TypeDeclarationSyntax classy)
    {
        var table = new List<ISymbol>();
        ...
        foreach (var item in classy.Members)
        {
            foreach (var attributeListSyntax in item.AttributeLists)
            {
                foreach (var attributeSyntax in attributeListSyntax.Attributes)
                {
                    ...
                    if (fullName == SourceGenerator.TableAttribute)
                    {
                        // return the parent class of the method
                        table.Add(MakeMember(item));
                    }
                }
            }
        }
        ...
        foreach (var attributeListSyntax in classy.AttributeLists)
        {
            foreach (var attributeSyntax in attributeListSyntax.Attributes)
            {
                ...
                {
                    // return the parent class of the method
                    table.AddRange(MakeMembers(attributeSyntax));
                    implementing.AddRange(MakeImplementedTypes(attributeSyntax));
                }
            }
        }

        return (table, implementing);
    }
